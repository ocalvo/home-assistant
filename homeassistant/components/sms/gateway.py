...
class Gateway:
    ...
    async def init_incoming_sms(self):
        """Initialize sms incoming notifications."""
        try:
        ...
        try:
            while remaining > 0:
                ...
                if all_parts_arrived or force:
                    ...
                    # folder = entry[0]["Folder"]
                    folder = 0
                    _LOGGER.debug(
                        "Deleting message: Folder=%i, Location=%i", folder, location
                    )
                    # try:
                    state_machine.DeleteSMS(Folder=folder, Location=location)
                    # except gammu.ERR_MEMORY_NOT_AVAILABLE:
                    #    _LOGGER.error("Cannot delete SMS")
                else:
...
async def create_sms_gateway(config, hass):
    ...
    try:
        ...
        await gateway.init_incoming_sms()
        await gateway.get_signal_quality_async()
        return gateway
